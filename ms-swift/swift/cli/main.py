...
def cli_main(route_mapping: Optional[Dict[str, str]] = None, is_megatron: bool = False) -> None:
    ...
    if not argv or argv[0] in {'-h', '--help'}:
        print('Usage: swift {sft,infer} [arguments]')
        return
    method_name = argv[0].replace('_', '-')
    if method_name not in route_mapping:
        commands = ', '.join(route_mapping)
        raise SystemExit(f'Unsupported command {method_name!r}; available commands: {commands}')
    argv = argv[1:]
    file_path = importlib.util.find_spec(route_mapping[method_name]).origin
    parse_yaml_args(argv)
    torchrun_args = get_torchrun_args()
    python_cmd = sys.executable
    if torchrun_args is None or (not is_megatron and method_name not in {'sft', 'infer'}):
