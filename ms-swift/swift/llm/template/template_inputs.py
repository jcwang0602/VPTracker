...
class InferRequest:
    ...
    visual_prompt: Dict[str, List[Any]] = field(default_factory=dict)
...
class StdTemplateInputs:
    ...
    def from_dict(cls, inputs: Dict[str, Any]) -> "StdTemplateInputs":
        ...
        for message in messages:
            if message["role"] == "tool_response":
                message["role"] = "tool"
            if message["role"] in {"tool_call", "tool"} and not isinstance(
                message["content"], str
            ):
    ...
    def remove_messages_media(messages: Messages) -> Dict[str, Any]:
        ...
        for message in messages:
            ...
            ) or (isinstance(content, dict) and "token_ids" in content):
                continue
            # List[Dict[str, Any]]
            new_content = ""
            for item in content:
...
class TemplateInputs:
    ...
    positive: List[StdTemplateInputs] = field(
    ...
    def _compat_rejected_response(inputs: Dict[str, Any]):
        ...
        if (
            isinstance(rejected_response, list)
            and rejected_response
            and isinstance(rejected_response[0], str)
        ):
            inputs["rejected_response"] = rejected_response
            return
        assert isinstance(rejected_response, str), (
            f"rejected_response: {rejected_response}"
        )
        ...
        if isinstance(rejected_response, str):
            if len(messages[idx:]) == 1:
                ...
                assert rejected_response != response, (
                    f"rejected_response: {rejected_response}, response: {response}"
                )
            rejected_response = [
                {"role": "assistant", "content": rejected_response}
            ]
        inputs["rejected_messages"] = (
            deepcopy(messages[:idx]) + rejected_response
        )
        ...
        for prefix in ["chosen"] + non_chosen_keys:
            if prefix == "chosen":
                std_inputs = {
                    ...
                    if not any(k.startswith(f"{p}_") for p in non_chosen_keys)
                }
            else:
                std_inputs = {
                    k[len(f"{prefix}_") :]: v
