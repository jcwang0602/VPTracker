...
class Template(ProcessorMixin):
    ...
    special_keys = ["images", "videos", "audios", "objects", "visual_prompt"]
    ...
    def _preprocess_inputs(
        ...
        if images:
            for i, image in enumerate(images):
                ...
                # 如果 i==0，则是模板图片，不需要绘制红色框
                if i == 0:
                    # print(f"DEBUG INFO: 处理模板")
                    try:
                        image_path = image["path"]
                    except:
                        image_path = image
                    try:
                        # print(f"DEBUG INFO: inputs: {inputs}")
                        # exit()
                        template_bbox = getattr(inputs, "visual_prompt", None)["template_bbox"]
                        template_scale = getattr(inputs, "visual_prompt", None)["template_scale"]
                        crop_img = sample_target(image_path, template_bbox, template_scale)
                        images[i] = Image.fromarray(crop_img)
                    except Exception as e:
                        print(f"DEBUG INFO: 处理模板失败: {e}", flush=True)
                        raise e
                    # TODO: 保存模板图片
                    # origin_img = Image.open(image_path)
                    # save_dir_path = "/share/wangjingchao/GLTracker/tmp/vp_images_train_template"
                    # os.makedirs(save_dir_path, exist_ok=True)
                    # save_path = os.path.join(
                    #     save_dir_path, os.path.basename(image_path)
                    # )
                    ...
                    # try:
                    if getattr(inputs, "visual_prompt", None)["enable"]:
                        if getattr(inputs, "visual_prompt", None)["phase"] == "train":
                            bbox = getattr(inputs, "visual_prompt", None)["search_bbox"]
                            scale = getattr(inputs, "visual_prompt", None)["scale"]
                            color = getattr(inputs, "visual_prompt", None)["color"]
                            width = getattr(inputs, "visual_prompt", None)["width"]
                            inbbox_ratio = getattr(inputs, "visual_prompt", None)["inbbox_ratio"]
                            # Train
                            images[i] = self._draw_bbox(
                                images[i],
                                bbox,
                                scale=scale,
                                color=color,
                                width=width,
                                add_noise=True,
                                inbbox_ratio=inbbox_ratio,
                            )
                            # TODO: 保存搜索图片
                            # save_dir_path = "/share/wangjingchao/GLTracker/tmp/vp_images_train_search"
                            # os.makedirs(save_dir_path, exist_ok=True)
                            # save_path = os.path.join(save_dir_path, os.path.basename(image_path))
                            # images[i] = self._draw_bbox(images[i], bbox, scale=scale, color=color, width=width, add_noise=True, inbbox_ratio=inbbox_ratio, save_path=save_path)
                        else:
                            # 测试阶段仅扩大，不添加扰动
                            bbox = getattr(inputs, "visual_prompt", None)["search_bbox"]
                            scale = getattr(inputs, "visual_prompt", None)["scale"]
                            color = getattr(inputs, "visual_prompt", None)["color"]
                            width = getattr(inputs, "visual_prompt", None)["width"]
                            # save_image = getattr(inputs, "visual_prompt", None)["save_image"]
                            save_image = False
                            if save_image:
                                video_name = image_path.split("/")[-3]
                                save_dir_path = os.path.join(
                                    "/mnt/shared-storage-user/mineru4s/jcwang/VPLT/vp_images_test",
                                    video_name,
                                )
                                os.makedirs(save_dir_path, exist_ok=True)
                                save_path = os.path.join(
                            ...
                            images[i] = self._draw_bbox(
                                ...
                                width=width,
                                add_noise=False,
                                save_path=save_path if save_image else None,
                            )
                    # except Exception as e:
                    #     print(f"DEBUG INFO: bbox: {bbox}", flush=True)
                    #     print(f"DEBUG INFO: scale: {scale}", flush=True)
                    #     print(f"DEBUG INFO: color: {color}", flush=True)
                    #     print(f"DEBUG INFO: width: {width}", flush=True)
                    #     print(f"DEBUG INFO: images[i]: {images[i].size}", flush=True)
                    #     print(f"DEBUG INFO: 处理搜索区域失败: {e}", flush=True)
                    #     raise e
        if inputs.objects:
        ...
        if self.mode == "vllm" and inputs.audios:
            ...
            inputs.audios = load_batch(
                inputs.audios,
                load_func=partial(load_audio, sampling_rate=sampling_rate, return_sr=True),
            )
        # exit()
    ...
    def _draw_bbox(
        ...
        inbbox_ratio: float = 0.8,
    ) -> Image.Image:
        # print(f"DEBUG INFO: 绘制矩形框")
        ...
        new_h = h * scale
        if add_noise:
            # 0.7 的概率目标必定在框内， 0.3 的概率目标必定没有完整的在框内
            if random.random() < inbbox_ratio:
                # 最大扰动幅度（确保目标仍然在框内）
                max_dx = (new_w - w) / 2
                max_dy = (new_h - h) / 2
                # 随机扰动目标中心，但保证目标仍在框内
                ...
                min_dy = (new_h - h) / 2
                # 计算最大，不能超出图片边界
                max_dx = img_width - new_w - min_dx
                max_dy = img_height - new_h - min_dy
                # 目标必定在框上或者框外
                dx = random.choice(
                    [
                        random.uniform(min_dx, max_dx),
                        random.uniform(-max_dx, -min_dx),
                    ]
                )
                dy = random.choice(
                    [
                        random.uniform(min_dy, max_dy),
                        random.uniform(-max_dy, -min_dy),
                    ]
                )
            new_cx = cx + dx
            new_cy = cy + dy
            ...
            new_cx = cx
            new_cy = cy
        # print(f"new_cx: {new_cx}, new_cy: {new_cy}")
        # 计算新框坐标
        new_x1 = int(new_cx - new_w / 2)
        new_y1 = int(new_cy - new_h / 2)
        new_x2 = int(new_cx + new_w / 2)
        new_y2 = int(new_cy + new_h / 2)
        ...
        # print(f"bbox:{bbox}, new_bbox: {new_bbox}")
        try:
            draw.rectangle(new_bbox, outline=color, width=width)
        except Exception as e:
            # 随机两个横坐标
            x1 = random.randint(1, img_width - 1)
            x2 = random.randint(1, img_width - 1)
            # 随机两个纵坐标
            y1 = random.randint(1, img_height - 1)
            y2 = random.randint(1, img_height - 1)
            # 保证 x1 < x2, y1 < y2
            x1, x2 = sorted([x1, x2])
            y1, y2 = sorted([y1, y2])
            print(f"DEBUG INFO: 绘制矩形框失败: {e}  bbox: {bbox}  new_bbox: {new_bbox} 绘制：{[x1, y1, x2, y2]}", flush=True)
            draw.rectangle([x1, y1, x2, y2], outline=color, width=width)

        if save_path:
            draw.rectangle(bbox, outline="green", width=width)
            image.save(save_path)
            print(f"INFO SAVE: save_path: {save_path}")
        return image
        ...
        for message in inputs.messages:
            ...
            for image in re.findall(pattern, content):
                ...
                if os.path.isfile(image):
                    ...
                    logger.warning_once(
                        f"Failed to parse image path: `{content}`.",
                        hash_id="<img></img>",
                    )
            ...
            "content": [
                {
                    ...
                    "top_logprobs": [{"index": idx, "logprob": logprobs[idx]} for idx in idxs],
                }
            ]
        }
    ...
    @staticmethod
    def _split_special_tokens(context_list: List[Context], loss_scale_list: List[float]) -> Tuple[List[Context], List[float]]:
    ...
    def _pre_tokenize_images(
        ...
        for context, loss_scale in zip(context_list, loss_scale_list):
            if context == "<image>" and inputs.is_multimodal and inputs.image_idx < len(inputs.images):
                ...
                inputs.image_idx += 1
                loss_scale = 0.0 if self.template_backend == "swift" else 1.0
            else:
                c_list = [context]
            res += c_list
        ...
        for media_type in ["image", "audio", "video"]:
            ...
            if medias:
                ...
                if num_new_tags > 0:
                    ...
                    logger.warning(f"num_media: {num_media}, num_media_tags: {num_media_tags}, total_content: {total_content}. We will only replace the frontmost media_tags while keeping the subsequent media_tags.")
    ...
    def _encode_context_list(
        ...
        if loss_scale_list is None:
            loss_scale_list = [0.0] * len(context_list)
    ...
    @staticmethod
    def _add_dynamic_eos(
        input_ids: List[int],
        labels: List[int],
        loss_scale: Optional[List[int]],
        suffix_tokens_id: List[int],
    ) -> None:
        ...
        while i < len(messages):
            ...
            if pre_role == "assistant" and role == "tool":
                ...
                while i + 1 < len(messages) and messages[i + 1]["role"] == "tool":
                    i += 1
                pre_message["content"], tool_content = self.agent_template._format_tool_responses(pre_content, messages[i_start : i + 1])
    ...
    def _swift_encode(self, inputs: StdTemplateInputs):
        ...
        if n_round > 1 and not self.template_meta.support_multi_round:
            logger.warning_once("The template does not support multi-round chat. Only use the last round of the conversation.")
        ...
        for i, (query_message, response_message) in enumerate(zip(inputs.messages[::2], inputs.messages[1::2])):
            ...
            if i < n_round - 1:
                ...
                endswith_stop_words = any(response_content.endswith(stop_word) for stop_word in template_meta.stop_words if isinstance(stop_word, str))
    ...
    def _encode_truncated(self, inputs: StdTemplateInputs):
        ...
        if self.max_length is not None and length > self.max_length:
            if self.truncation_strategy in {"right", "left"}:
                ...
                raise MaxLengthError(f"Current length of row({length}) is larger than the max_length({self.max_length}).")
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        if self.template_meta.template_type == "dummy" and self.use_chat_template and not self.is_training and self.task_type == "causal_lm":
            template_backend = "jinja"
            logger.info_once(f"Setting template_backend: {template_backend}")
        res_context_list, loss_scale_list, answer_len = self._swift_encode(inputs) if template_backend == "swift" else self._jinja_encode(inputs)
        encoded = {}
        if self.is_encoder_decoder or self.mode == "gkd":
            ...
            for key, _slice in zip(
                ...
                context_list, loss_scale = self._simplify_context_list(res_context_list[_slice], loss_scale_list[_slice], inputs)
        ...
        self._add_dynamic_eos(
            input_ids,
            labels,
            loss_scale,
            self._encode_context_list(self.template_meta.suffix)[0],
        )
    ...
    def get_generate_ids(
        self,
        generate_ids: Union[torch.Tensor, List[int]],
    ...
    def set_mode(
        self,
        mode: Literal["pt", "vllm", "lmdeploy", "sglang", "train", "rlhf", "kto", "gkd"],
    ) -> None:
        self.mode = mode
    ...
    def _kto_data_collator(self, batch: List[Dict[str, Any]], *, padding_to: Optional[int] = None) -> Dict[str, Any]:
        ...
        res = {
            **{f"completion_{k}": v for k, v in res.items()},
            **{f"KL_completion_{k}": v for k, v in kl_res.items()},
        }
    ...
    def _embedding_data_collator(self, batch: List[Dict[str, Any]], *, padding_to: Optional[int] = None) -> Dict[str, Any]:
        labels = []
        new_batch = []
        for b in batch:
            if "input_ids" in b:
    ...
    def _reranker_data_collator(self, batch: List[Dict[str, Any]], *, padding_to: Optional[int] = None) -> Dict[str, Any]:
        if self.is_training:
            ...
            for b in batch:
                ...
                for i in random.sample(range(positive_num), max_positive):
                    ...
                    for j in random.sample(range(negative_num), max_negative):
                        new_batch.append({key: b[key][j + positive_num] for key in b.keys() if isinstance(b[key], list) and b[key][j + positive_num] is not None})
    ...
    def _seq_cls_data_collator(self, batch: List[Dict[str, Any]], *, padding_to: Optional[int] = None) -> Dict[str, Any]:
    ...
    def _data_collator(self, batch: List[Dict[str, Any]], *, padding_to: Optional[int] = None) -> Dict[str, Any]:
        ...
        pad_values = [self.tokenizer.pad_token_id, 0.0, 0, -100, 0.0, 0.0, 0, 0]
            ...
            if self.padding_free:
                ...
                if cp_size > 1:
                    ...
                    if position_ids.ndim == 3:  # compat mrope
                        extended_position_ids = extended_position_ids[None, None, :].expand(position_ids.shape[0], 1, -1)
                ...
                res["attention_mask"] = torch.tril(torch.ones((len(seq_lens), seq_len, seq_len), dtype=torch.bool)).view(len(seq_lens), 1, seq_len, seq_len)
        ...
        for key, pad_value in zip(keys, pad_values):
            ...
            if self.use_megatron and not self.padding_free and key == "attention_mask":
                continue
            if padding_to is not None and not (self.padding_free and key == "position_ids" and self.sequence_parallel_size > 1):
                padding_len = padding_to - seq_lens[0]
                if padding_len > 0:
                    res[key][0] = F.pad(
                        res[key][0],
                        (0, padding_len) if padding_right else (padding_len, 0),
                        "constant",
                        pad_value,
                    )
    ...
    def print_inputs(self, inputs: Dict[str, Any]) -> None:
                    ...
                    for prefix in prefixes:
                        if key.startswith(prefix):
                            ...
                            if base_key in [
                                "input_ids",
                                "labels",
                            ] or base_key.rstrip("0123456789_") in [
                                "input",
                                "labels",
                            ]:
        ...
        for key in keys_to_check:
            ...
            if key.endswith("labels") and self.task_type in {
                "reranker",
                "generative_reranker",
            }:
                continue
    ...
    def _pad_sequence(self, sequences: List[torch.Tensor], padding_value: float = 0.0) -> torch.Tensor:
    ...
    def _get_inputs_embeds_hf(inputs_embeds, inputs, visual, processor, config):
        ...
        if pixel_values is None and pixel_values_videos is None:  # plain-text
            ...
            inputs_embeds = inputs_embeds + image_embeds.mean().to(device=inputs_embeds.device) * 0.0
        else:
